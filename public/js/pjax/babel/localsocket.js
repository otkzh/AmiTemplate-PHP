...
define = typeof define === 'function' && define.amd ? define : function () {
    ...
    return function define(m, rs, f) {
        return !f ? void define(name, m, rs) : void f.apply(this, rs.map(function (r) {
            switch (r) {
                case 'require':
                    {
                ...
                case 'exports':
                    {
                        return m.indexOf('/') === -1 ? workspace[m] = typeof exports === 'undefined' ? self[m] = self[m] || {} : exports : workspace[m] = workspace.hasOwnProperty(m) ? workspace[m] : {};
                    }
                default:
                    {
                        return r.slice(-2) === '.d' && {} || workspace.hasOwnProperty(r) && workspace[r] || typeof require === 'function' && require(r) || self[r];
                    }
            }
        }));
    };
...
define('src/layer/data/constraint/values', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/lib/noop', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/layer/domain/dao/module/builder', ['require', 'exports', 'src/layer/data/constraint/values', 'src/lib/noop'], function (require, exports, values_1, noop_1) {
    'use strict';
    ...
    function build(source, factory, update) {
        ...
        if (typeof source[exports.SCHEMA.KEY.NAME] !== 'string') throw new TypeError('LocalSocket: Invalid key: ' + source[exports.SCHEMA.KEY.NAME]);
        var descmap = Object.assign(Object.keys(dao).filter(values_1.isValidName).filter(values_1.isValidValue(dao)).reduce(function (map, prop) {
            ...
            map[prop] = {
                ...
                set: function set(newVal) {
                    ...
                    if (!values_1.isValidValue(source)(prop)) return;
...
define('src/layer/domain/dao/api', ['require', 'exports', 'src/layer/domain/dao/module/builder'], function (require, exports, builder_1) {
    'use strict';
...
define('src/layer/infrastructure/indexeddb/module/global', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/layer/infrastructure/indexeddb/model/event', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/layer/infrastructure/indexeddb/model/access', ['require', 'exports', 'spica', 'src/layer/infrastructure/indexeddb/module/global', 'src/layer/infrastructure/indexeddb/model/event'], function (require, exports, spica_1, global_1, event_1) {
    'use strict';
    ...
    function close(name) {
        ...
        if (StateMap.get(name) instanceof State.Success) return StateMap.get(name).end();
        if (StateMap.has(name)) return;
        void handleFromInitialState(new State.Initial(name));
    }
    exports.close = close;
    function destroy(name) {
        ...
        if (StateMap.get(name) instanceof State.Success) return StateMap.get(name).destroy();
        if (StateMap.has(name)) return;
        void handleFromInitialState(new State.Initial(name));
    }
    ...
    function handleFromInitialState(_a, version) {
        ...
        function handleFromBlockedState(_a) {
            var database = _a.database;
            void IDBEventObserver.emit([database, event_1.IDBEventType.block], new event_1.IDBEvent(event_1.IDBEventType.block, database));
        }
        function handleFromUpgradeState(_a) {
            var database = _a.database,
                session = _a.session;
            var db = session.transaction.db;
            var _b = exports.ConfigMap.get(database),
                make = _b.make,
                destroy = _b.destroy;
            try {
                if (make(db)) {
        ...
        function handleFromSuccessState(state) {
            ...
            connection.onversionchange = function (_a) {
                ...
                if (!newVersion) {
                    ...
                    void IDBEventObserver.emit([database, event_1.IDBEventType.destroy], new event_1.IDBEvent(event_1.IDBEventType.destroy, database));
                }
            ...
            connection.onclose = function () {
                return void clear(), void IDBEventObserver.emit([database, event_1.IDBEventType.destroy], new event_1.IDBEvent(event_1.IDBEventType.destroy, database)), StateMap.get(database) === state ? void handleFromEndState(new State.End(database)) : void 0;
            };
                ...
                case 0:
                    {
                        ...
                        void IDBEventObserver.emit([database, event_1.IDBEventType.connect], new event_1.IDBEvent(event_1.IDBEventType.connect, database));
        ...
        function handleFromErrorState(_a) {
            ...
            void IDBEventObserver.emit([database, event_1.IDBEventType.error], new event_1.IDBEvent(event_1.IDBEventType.error, database));
        ...
        function handleFromAbortState(_a) {
            var database = _a.database,
                ...
                event = _a.event;
            void event.preventDefault();
            void IDBEventObserver.emit([database, event_1.IDBEventType.abort], new event_1.IDBEvent(event_1.IDBEventType.abort, database));
            var destroy = exports.ConfigMap.get(database).destroy;
            if (destroy(error, event)) {
                return void handleFromDestroyState(new State.Destroy(database));
            } else {
        ...
        function handleFromCrashState(_a) {
            var database = _a.database,
                error = _a.error;
            void IDBEventObserver.emit([database, event_1.IDBEventType.crash], new event_1.IDBEvent(event_1.IDBEventType.crash, database));
            var destroy = exports.ConfigMap.get(database).destroy;
        ...
        function handleFromDestroyState(_a) {
            ...
            deleteRequest.onsuccess = function () {
                return void requests.delete(database), void IDBEventObserver.emit([database, event_1.IDBEventType.destroy], new event_1.IDBEvent(event_1.IDBEventType.destroy, database)), void handleFromEndState(new State.End(database));
            };
        ...
        function handleFromEndState(_a, version) {
            ...
            switch (CommandMap.get(database)) {
                ...
                case 1:
                    return void CommandMap.delete(database), void exports.ConfigMap.delete(database), void IDBEventObserver.emit([database, event_1.IDBEventType.disconnect], new event_1.IDBEvent(event_1.IDBEventType.disconnect, database));
                case 2:
                    return void CommandMap.delete(database), void exports.ConfigMap.delete(database), void IDBEventObserver.emit([database, event_1.IDBEventType.disconnect], new event_1.IDBEvent(event_1.IDBEventType.disconnect, database));
            }
            throw new TypeError('LocalSocket: Invalid command ' + CommandMap.get(database) + '.');
...
define('src/layer/infrastructure/indexeddb/api', ['require', 'exports', 'src/layer/infrastructure/indexeddb/module/global', 'src/layer/infrastructure/indexeddb/model/access', 'src/layer/infrastructure/indexeddb/model/event'], function (require, exports, global_2, access_1, event_2) {
    'use strict';
...
define('src/layer/data/constraint/types', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/layer/data/schema/event', ['require', 'exports', 'spica'], function (require, exports, spica_2) {
    'use strict';
    ...
    var EventRecord = function () {
        function EventRecord(key, value, date, type) {
            ...
            this.value = value;
            if (_typeof(this.value) !== 'object' || !this.value) throw new TypeError('LocalSocket: EventRecord: Invalid event value: ' + this.value);
            this.date = date;
            ...
            if (this.type === exports.EventType.put && this.attr.length === 0) throw new TypeError('LocalSocket: EventRecord: Invalid event attr with ' + this.type + ': ' + this.attr);
            if (this.type !== exports.EventType.put && this.attr.length !== 0) throw new TypeError('LocalSocket: EventRecord: Invalid event attr with ' + this.type + ': ' + this.attr);
            switch (type) {
...
define('src/layer/data/store/event', ['require', 'exports', 'spica', 'src/layer/infrastructure/indexeddb/api', 'src/layer/data/constraint/types', 'src/layer/data/schema/event', 'src/layer/data/schema/event', 'src/lib/noop'], function (require, exports, spica_3, api_1, types_1, event_3, Schema, noop_2) {
    'use strict';
    ...
    var EventStore = function () {
        function EventStore(database, name) {
            ...
            void this.memory.events.exec.monitor([], function (_a) {
                ...
                if (event instanceof event_3.UnsavedEventRecord) return;
                if (event.date <= states.date.get(event.key) && event.id <= states.id.get(event.key)) return;
                void _this.events.load.emit([event.key, event.attr, event.type], new EventStore.Event(event.type, event.id, event.key, event.attr, event.date));
            });
            void this.memory.events.exec.monitor([], function (_a) {
                var sub = _a[1];
                var event = sub(void 0);
            ...
            return {
                make: function make(db) {
                    ...
                    if (!store.indexNames.contains(event_3.EventRecordFields.surrogateKeyDateField)) {
                        void store.createIndex(event_3.EventRecordFields.surrogateKeyDateField, [event_3.EventRecordFields.key, event_3.EventRecordFields.date]);
                    }
                    return true;
                },
                verify: function verify(db) {
        ...
        EventStore.prototype.update = function (key, attr, id) {
            return typeof id === 'string' && typeof attr === 'string' ? void this.memory.cast([key, attr, id], void 0) : typeof attr === 'string' ? void this.memory.cast([key, attr], void 0) : void this.memory.cast([key], void 0);
        };
                    ...
                    case false:
                        return cb === noop_2.noop ? new Promise(function (resolve) {
                            ...
                            return void (timeout > 0 ? void (void _this.get(key), void setTimeout(function () {
                                return resolve([key, new Error()]);
                            })) : void 0, void _this.syncWaits.once([key], function (err) {
                                return void resolve([key, err]);
                            }));
                        });
                    default:
                        {
        ...
        EventStore.prototype.fetch = function (key) {
            ...
            return void this.cursor(key, event_3.EventRecordFields.key, api_1.IDBCursorDirection.prev, api_1.IDBTransactionMode.readonly, function (cursor, err) {
                if (err) return void _this.syncWaits.emit([key], err);
                if (!cursor || cursor.value.date < _this.meta(key).date) {
                    void Array.from(savedEvents.reduceRight(function (acc, e) {
                        ...
                        void _this.memory.register([e.key, e.attr, spica_3.sqid(e.id)], function () {
                            return e;
                        });
                    });
                    ...
                    if (_this.memory.refs([event_4.key, event_4.attr, spica_3.sqid(event_4.id)]).length > 0) return;
                    void savedEvents.unshift(new event_3.SavedEventRecord(event_4.id, event_4.key, event_4.value, event_4.type, event_4.date));
                    if (event_4.type !== EventStore.EventType.put) return;
        ...
        EventStore.prototype.add = function (event, tx) {
            var _this = this;
            void this.events_.access.emit([event.key, event.attr, event.type], new InternalEvent(event.type, types_1.IdNumber(0), event.key, event.attr));
            if (!(event instanceof event_3.UnsavedEventRecord)) throw new Error('LocalSocket: Cannot add a saved event: ' + JSON.stringify(event));
            void this.sync([event.key]);
            switch (event.type) {
                case EventStore.EventType.put:
                    {
                        void this.memory.terminate([event.key, event.attr, spica_3.sqid(0)]);
                        break;
                    }
                case EventStore.EventType.delete:
                case EventStore.EventType.snapshot:
            ...
            var terminate = this.memory.register([event.key, event.attr, spica_3.sqid(0), spica_3.sqid()], function () {
                return event;
            });
            ...
            return void new Promise(function (resolve, reject) {
                ...
                var cont = function cont(tx) {
                    var active = function active() {
                        return _this.memory.refs([event.key, event.attr, spica_3.sqid(0)]).reduce(function (acc, _a) {
                            var s = _a[1];
                    ...
                    tx.oncomplete = function () {
                        ...
                        void _this.memory.register([savedEvent.key, savedEvent.attr, spica_3.sqid(savedEvent.id)], function () {
                            return savedEvent;
                        });
                        void _this.events.save.emit([savedEvent.key, savedEvent.attr, savedEvent.type], new EventStore.Event(savedEvent.type, savedEvent.id, savedEvent.key, savedEvent.attr, event.date));
                ...
                return void _this.events.loss.emit([event.key, event.attr, event.type], new EventStore.Event(event.type, types_1.IdNumber(0), event.key, event.attr, event.date));
            });
        };
        ...
        EventStore.prototype.snapshot = function (key) {
            ...
            return void api_1.listen(this.database)(function (db) {
                if (!_this.syncState.get(key)) return;
                ...
                req.onsuccess = function () {
                    ...
                    if (!cursor || cursor.value.type !== EventStore.EventType.put) {
                        ...
                        if (composedEvent instanceof event_3.SavedEventRecord) return;
        ...
        EventStore.prototype.clean = function (until, key) {
            ...
            return void this.cursor(key ? api_1.IDBKeyRange.bound([key, 0], [key, until]) : api_1.IDBKeyRange.upperBound(until), key ? event_3.EventRecordFields.surrogateKeyDateField : event_3.EventRecordFields.date, api_1.IDBCursorDirection.prev, api_1.IDBTransactionMode.readwrite, function (cursor) {
                if (!cursor) {
                    return void removedEvents.reduce(function (_, event) {
                        return void _this.memory.terminate([event.key, event.attr, spica_3.sqid(event.id)]);
                    }, void 0);
                } else {
...
define('src/layer/domain/indexeddb/model/socket/data', ['require', 'exports', 'src/layer/data/store/event'], function (require, exports, event_7) {
    'use strict';
...
define('src/layer/data/store/key-value', ['require', 'exports', 'spica', 'src/layer/infrastructure/indexeddb/api', 'src/lib/noop'], function (require, exports, spica_4, api_2, noop_3) {
    'use strict';
...
define('src/layer/domain/indexeddb/model/socket/access', ['require', 'exports', 'src/layer/data/store/key-value', 'src/layer/data/store/event'], function (require, exports, key_value_1, event_8) {
    'use strict';
...
define('src/layer/domain/indexeddb/model/socket/expiry', ['require', 'exports', 'src/layer/infrastructure/indexeddb/api', 'src/layer/data/store/key-value', 'src/layer/data/store/event'], function (require, exports, api_3, key_value_2, event_9) {
    'use strict';
    ...
    var ExpiryStore = function (_super) {
        ...
        function ExpiryStore(database, store, data, expiries) {
            ...
            var schedule = function schedule(date) {
                ...
                void api_3.event.once([database, api_3.IDBEventType.destroy], function () {
...
define('src/layer/domain/indexeddb/model/socket', ['require', 'exports', 'spica', 'src/layer/infrastructure/indexeddb/api', 'src/layer/domain/indexeddb/model/socket/data', 'src/layer/domain/indexeddb/model/socket/access', 'src/layer/domain/indexeddb/model/socket/expiry', 'src/lib/noop'], function (require, exports, spica_5, api_4, data_1, access_2, expiry_1, noop_4) {
    'use strict';

    var SocketStore = function () {
        function SocketStore(database, _destroy, expiry) {
            var _this = this;
            if (expiry === void 0) {
                expiry = Infinity;
            }
            ...
            void api_4.open(database, {
                ...
                destroy: function destroy(err, ev) {
                    return data_1.DataStore.configure().destroy(err, ev) && access_2.AccessStore.configure().destroy(err, ev) && expiry_1.ExpiryStore.configure().destroy(err, ev) && _destroy(err, ev);
                }
            });
            this.schema = new Schema(this, this.expiries);
            void api_4.event.on([database, api_4.IDBEventType.destroy, this.uuid], function () {
        ...
        SocketStore.prototype.destroy = function () {
            void api_4.event.off([this.database, api_4.IDBEventType.destroy, this.uuid]);
    ...
    var Schema = function () {
        ...
        Schema.prototype.bind = function () {
            ...
            this.data.events.load.monitor([], function (ev) {
                return _this.store_.events.load.emit([ev.key, ev.attr, ev.type], ev);
            });
            this.data.events.save.monitor([], function (ev) {
                return _this.store_.events.save.emit([ev.key, ev.attr, ev.type], ev);
            });
            this.data.events.loss.monitor([], function (ev) {
                return _this.store_.events.loss.emit([ev.key, ev.attr, ev.type], ev);
            });
...
define('src/layer/infrastructure/webstorage/module/global', ['require', 'exports', 'spica'], function (require, exports, spica_6) {
    'use strict';
...
define('src/layer/infrastructure/webstorage/model/event', ['require', 'exports', 'spica', 'src/layer/infrastructure/webstorage/module/global'], function (require, exports, spica_7, global_3) {
    'use strict';
...
define('src/layer/infrastructure/webstorage/api', ['require', 'exports', 'src/layer/infrastructure/webstorage/module/global', 'src/layer/infrastructure/webstorage/model/event'], function (require, exports, global_4, event_10) {
    'use strict';
...
define('src/layer/domain/webstorage/service/event', ['require', 'exports', 'spica', 'src/layer/infrastructure/webstorage/api'], function (require, exports, spica_8, api_5) {
    'use strict';
...
define('src/layer/domain/webstorage/service/storage', ['require', 'exports'], function (require, exports) {
    'use strict';
...
define('src/layer/domain/webstorage/repository/port', ['require', 'exports', 'spica', 'src/layer/domain/dao/api', 'src/layer/domain/webstorage/service/event', 'src/layer/infrastructure/webstorage/api', 'src/layer/domain/webstorage/service/storage'], function (require, exports, spica_9, api_6, event_11, api_7, storage_1) {
    'use strict';
    ...
    function port(name, storage, factory, log) {
        ...
        if (log === void 0) {
            log = {
                update: function update(_name) {},
                delete: function _delete(_name) {}
            };
        }
    ...
    var Port = function () {
        ...
        Port.prototype.link = function () {
            var _this = this;
            if (this.cache.has(this.name)) return this.cache.get(this.name);
            ...
            var dao = api_6.build(source, this.factory, function (attr, newValue, oldValue) {
                ...
                void source.__event.emit([event.type, event.attr], event);
                void _this.events.send.emit([event.attr], event);
            });
            var subscriber = function subscriber(_a) {
...
define('src/layer/domain/webstorage/api', ['require', 'exports', 'src/layer/infrastructure/webstorage/api', 'src/layer/domain/webstorage/service/event', 'src/layer/domain/webstorage/repository/port'], function (require, exports, api_8, event_12, port_1) {
    'use strict';
...
define('src/layer/domain/indexeddb/repository/socket', ['require', 'exports', 'spica', 'src/layer/domain/dao/api', 'src/layer/domain/indexeddb/model/socket', 'src/layer/infrastructure/webstorage/api', 'src/layer/domain/webstorage/api'], function (require, exports, spica_10, api_9, socket_1, api_10, api_11) {
    'use strict';
    ...
    var Socket = function (_super) {
        ...
        function Socket(database, factory, expiry, destroy) {
            ...
            void this.port.__event.on([api_11.WebStorageEventType.recv, 'msgs'], function () {
                    ...
                    case socket_1.SocketStore.EventType.put:
                        {
                            ...
                            source[attr] = newVal;
                            void source.__event.emit([api_11.WebStorageEventType.recv, attr], new api_11.WebStorageEvent(api_11.WebStorageEventType.recv, key, attr, newVal, oldVal));
                            return;
                        }
        ...
        Socket.prototype.link = function (key, expiry) {
            ...
            return this.links.has(key) ? this.links.get(key) : this.links.set(key, api_9.build(Object.defineProperties((void this.sources.set(key, spica_10.clone({}, this.get(key))), this.sources.get(key)), {
                ...
                return void _this.add(new socket_1.SocketStore.Record(key, (_a = {}, _a[attr] = newValue, _a))), void _this.sources.get(key).__event.emit([api_11.WebStorageEventType.send, attr], new api_11.WebStorageEvent(api_11.WebStorageEventType.send, key, attr, newValue, oldValue));
                var _a;
            })).get(key);
        };
...
define('src/layer/domain/indexeddb/service/event', ['require', 'exports', 'src/layer/infrastructure/indexeddb/api'], function (require, exports, api_12) {
    'use strict';

    exports.event = api_12.event;
    exports.IDBEventType = api_12.IDBEventType;
});
define('src/layer/domain/indexeddb/api', ['require', 'exports', 'src/layer/domain/indexeddb/repository/socket', 'src/layer/domain/indexeddb/service/event'], function (require, exports, socket_2, event_13) {
    'use strict';
...
define('src/layer/application/api', ['require', 'exports', 'src/layer/domain/indexeddb/api', 'src/layer/domain/webstorage/api', 'src/layer/domain/indexeddb/api', 'src/layer/domain/webstorage/api', 'src/layer/domain/webstorage/api'], function (require, exports, api_13, api_14, api_15, api_16, api_17) {
    'use strict';
...
define('src/layer/interface/api', ['require', 'exports', 'src/layer/application/api'], function (require, exports, api_18) {
    'use strict';
...
define('src/export', ['require', 'exports', 'src/layer/interface/api'], function (require, exports, api_19) {
    'use strict';
...
define('localsocket', ['require', 'exports', 'src/export', 'src/export', './index', './src/import'], function (require, exports, export_1, export_2) {
    'use strict';
